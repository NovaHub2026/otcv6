...
Status: APPROVED — every material finding fixed and watched failing; local gate exit 0 and hosted CI green on the same tree; residue in GitHub Issues #1–#14; merged into `main` 2026-09-02
...
Fixed on `feature/out-of-band-audit`, one commit per area, each fix watched
failing on the auditor's own plant or probe before it was recorded as closed.
Six implementation agents (engine, runtime, guardrails, laboratory, API and
web, the architecture document) worked in the same worktree on disjoint file
sets; the orchestrator committed by explicit file list. What was not closed is
a GitHub Issue — the migration `GOVERNANCE.md` §42 requires (a7-04) — and is
named in `CURRENT_STATE.md`.

| Finding                                                       | Closed by                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| a3-01, a3-02                                                  | `fix(engine)`: the interior-snapshot mirror harness; plant 20 fails 11 of 19 tests                                                                                                                                                                                                                                                                   |
| a3-03 … a3-07, a3-12, a3-13, a3-10, a3-08                     | `fix(engine)`: constant cascade accepted; id length 51; the record carries the drawn target, retreats and clamp; alt-crypto depth 7; refusals at the stage they name; named literals                                                                                                                                                                 |
| a3-09, a3-11, a3-14                                           | recorded, no change (parity is not a leak; surface pruning out of scope; unit watchdog — `test:unit` now writes a JSON report instead)                                                                                                                                                                                                               |
| a2-01 … a2-12                                                 | `fix(guardrails)`: one tokenizer with a 51-case corpus, new bans, one walker, `tools/sim` scanned, structural publishing-key check, the loop detector, exact status vocabulary, recursive surface, 33 mutations; residue in Issue #12                                                                                                                |
| a1-01, a1-02, a1-03, a1-06, a1-07, a1-08                      | `fix(gate)` and `fix(lab)`: `sampledCatalogue` yields; the round-trip guard fails a file by name at 30 s (watched failing on a 35 s plant); `disableConsoleIntercept` in the projects; `default` reporter; heap in `test:stat`; JSON unit report; no cancel on `main`; the decision-log rule — closed for good when a hosted run is green (Issue #1) |
| a1-04                                                         | the two 25–31 s tests moved to the statistical project                                                                                                                                                                                                                                                                                               |
| a1-05                                                         | `fix(api,web)`: `next` spawned directly, process group killed                                                                                                                                                                                                                                                                                        |
| a4-01                                                         | `fix(lab)`: the unconditional family, the gate MDE per horizon, the sensitivity tripwire; the honest residue is Issue #10                                                                                                                                                                                                                            |
| a4-02, a4-03, a4-04, a4-07, a4-08, a4-11, a4-12, a4-09, a4-10 | `fix(lab)`: the sweeping stride, the hand-built look-ahead test, the learned family in the standing verdict, ratios gated on their precondition, ties confused, boundary instants, VALIDATION.md refreshed, evidence summaries re-derived                                                                                                            |
| a4-05, a4-06                                                  | B-029 restated (Issue #4); the realism header says what the record supports                                                                                                                                                                                                                                                                          |
| a5-01 … a5-07, a5-09 … a5-12                                  | `fix(runtime)`: no partial bar stored; retention boundary tested; leader retries and loses the lease; clock inside the transaction and one batch rule; rejections not throws; the registry's temp names, queue and id check; feed on empty history; fsync and shared WAL enable; schema versions; leading edge and `to`                              |
| a5-08                                                         | Issue #11 (a product decision to record first)                                                                                                                                                                                                                                                                                                       |
| a6-01 … a6-05, a6-07 … a6-09, a6-11 … a6-18                   | `fix(api,web)`: the bearer token, JSON-only writes, loopback bind; controller-side rename order; a real skip; 400 for a future sequence; one stall line per kind; validation by name; one shutdown path; honest reconnection; `/health` in the shell; boot nonces; doc drift                                                                         |
| a6-06                                                         | `feat(engine,api)`: `onStage`                                                                                                                                                                                                                                                                                                                        |
| a6-10                                                         | the poll stops on a 404 and says why; `_jobs.json` not built                                                                                                                                                                                                                                                                                         |
| a7-01, a7-05, a7-11                                           | `docs`: PH-19's approval corrected in place; the 46-row closure table; PH-19.3–19.5 say no plants were recorded                                                                                                                                                                                                                                      |
| a7-02                                                         | recorded in `CURRENT_STATE.md`; the feature branch is the other session's (rebase instructions in `SESSION_HANDOFF.md`)                                                                                                                                                                                                                              |
| a7-03                                                         | B-021 consolidated with the four attributions and the mechanism (Issue #1)                                                                                                                                                                                                                                                                           |
| a7-04, a7-06                                                  | fourteen Issues; `BACKLOG.md` is the archive                                                                                                                                                                                                                                                                                                         |
| a7-07, a7-12, a7-13, a7-15, a7-19, a7-22, a7-23 (part)        | `docs`: `CLAUDE.md` commands and timings, `PROJECT_CONTEXT.md`, ADR-0008 annotated, the index, the evidence table, pointers instead of copies for the registration timing                                                                                                                                                                            |
| a7-08                                                         | `docs(architecture)`: `MULTI_NODE_AND_OPERATIONS.md`; B-023 closed                                                                                                                                                                                                                                                                                   |
| a7-09, a7-10                                                  | `CURRENT_STATE.md` limitations and the roadmap rewritten to the present                                                                                                                                                                                                                                                                              |
| a7-14                                                         | Issue #2 (coverage to be re-measured)                                                                                                                                                                                                                                                                                                                |
| a7-16, a7-21                                                  | Issues #14 and #13, for the Human Owner                                                                                                                                                                                                                                                                                                              |
| a7-17, a7-18, a7-20, a7-24                                    | the index says merge commits; the handoff carries a HEAD; the audit-6 worktrees removed; `cancel-in-progress` only off `main`                                                                                                                                                                                                                        |
| a7-25                                                         | Issue #9 — a phase's worth of composition, proposed as PH-22                                                                                                                                                                                                                                                                                         |

## 8. Verification

`npm run gate` on `feature/out-of-band-audit` at `023f694`, 2026-09-02, 19:06 to
19:38 UTC — **exit 0**:

| Check                                | Result                                                                                                                               |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ |
| format, build, both typechecks, lint | exit 0 each, in that order                                                                                                           |
| unit                                 | 88 files, 2,165 tests (1,866 before the audit)                                                                                       |
| statistical                          | 40 files, 265 tests; 6 browser tests skipped (no Chromium on this host)                                                              |
| unhandled errors                     | none                                                                                                                                 |
| worst RPC round trip                 | 6.6 s (the guard fails a file at 30 s; the suite's worst was 18.4 s before the last fix, 92 s on the hosted runner before the audit) |
