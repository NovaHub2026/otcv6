...
solucionar todo para poder implementar el motor en un broker real"_ — and the
clarification that followed: _"esto es el motor de OTC de un broker, nosotros no
estamos construyendo el broker, solo debemos dejar preparado el motor para
integrarlo al broker de opciones binarias."_ Cycle 10 is therefore shaped as the
cycle after which a binary-options broker integrates this engine, and its three
phases are chosen from what the record says stands between the engine and that
integration — not from what would be most interesting to build. The
...
venue from the release, on one machine, with its record durable across
restarts; asks it for the entry and expiry prices its own settlement needs and
can prove them against the published commitments; codes against an API
contract that is frozen, versioned and testable from outside; operates it from
a runbook; and reads a catalogue measured whole under production composition.
Accounts, positions, payout, money, risk and the trader's user interface are
the broker's and are **not built here** — `packages/trading` remains what it
is, a reference implementation of deterministic settlement a broker may embed,
never a venue route. Three things in the report's ranking are not required for
the integration and are deferred, recorded rather than dropped: the engine's
next stylised facts (rank 3), the hosted multi-node composition (rank 6) and
jumps and volume (rank 7). Two things cannot be finished inside this repository
at all and are the Human Owner's under `GOVERNANCE.md` §5.1: whatever binds
them outside it — licensing, regulation, the broker's contracts and money —
and the two Governance amendments open as Issues #3 and #14. The Cycle Audit
at the end of the cycle is not optional and is not shortened because the cycle
is the last one planned; after it closes, the loop continues in maintenance
...
of the stream and re-derive entry and expiry prices from it. A broker should
instead be able to ask the engine: the price at a sequence, the price at an
instant under the venue's own rule for the candle edge, and a proof that the
answer is in the commitment chain (INV-009) — so that a dispute is settled by
the record, not by whose copy was better. The rest of the phase makes the
boundary something a broker can code against and test: the API contract frozen
and versioned, an executable conformance suite the broker runs against its own
deployment, a reference client in the package exercised by the release, and
...
| PH-29.4  | The reference client and settlement library: exercised by the release, #11 decided     | ACTIVE   |
...
files in the tree; the stream at scale for the broker's own frontend — one
multiplexed stream (Issue #16) — and the operator's panel honest about
`resumesAt` holes and a retired market (a8-12 carried); ten thousand observers
...
| PH-30.2  | The stream at scale: one multiplexed stream (#16); the operator's panel with holes, retired | PLANNED |
