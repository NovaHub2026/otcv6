...
| INV-010 Private generator state                        | Enforced | `keyring.test.ts` (key material is redacted in JSON, string and inspect forms), `engine.test.ts` (a snapshot carries no key material), `lease.test.ts` ; `publishingKey.test.ts` (structural refusal, transitive signing path), `singleWriter.test.ts` (a follower cannot reach the engine or key material)                                                                                                                                                                                                    |
