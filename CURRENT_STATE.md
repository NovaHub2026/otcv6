...
| Last approved subphase | PH-24.16 — Empujar progresivo y dirección        |

**PH-24 is active; sixteen subphases approved (PH-24.1–24.16).** The Lab is
the controls the Human Owner asked for — pushes by buttons that begin at the
click and arrive progressively at a chosen pace, a sustained direction (sube /
baja) in runs at the market's pace, and the exact close of the current candle
— on one engine per deployment (ADR-0018), with a panel that recovers by itself
and a gate that never touches it. The next Lab item awaits the Human Owner.
