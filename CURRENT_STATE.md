...
| Last approved subphase | PH-38.2 — The past is declared, and only what can be evidenced           |
...
| `88292d0` | **Cycle Audit 12 merge**                         | **red** — Quality Gate green (10 min); Statistical Gate failed at 1h13m on one test, `servedRecord.stat.test.ts` "is the same for two observers…", at `expect(alone.horizons.length).toBeGreaterThan(0)`. The local phase gate on the same code was green, so this is a finding about the gate. **Diagnosed, not excused:** the two assertions above it passed, so the record held its full 1,800 ticks — what was empty was `verdict.sensitivity`, because `battery.ts` skips any horizon whose evaluation split has no decided outcome, and every horizon was skipped. The test fixes a tick _count_ and asserts a property that depends on the _market time_ those ticks span, which it never controls: in a high-activity regime 1,800 ticks cover less than one 30 s horizon plus its warmup. A statistical assertion that can fail on which regime the venue happened to be in is a defect, not a flake (CLAUDE.md §5) |
