...
`v1.0.0` was tagged and pushed at 15:54Z; the Statistical Gate on that push
failed at 17:57Z on the Lab's browser suite. `RELEASE-1.0.0.md` was written
claiming corroboration it did not yet have, and is corrected. The tagged code is
the gated code (no code differs between `6f1efa9`, which passed the local phase
gate, and the merge), but a red hosted run on a green local gate is a finding,
and it is recorded as one rather than explained away.
