...
/**
 * `npm run conformance -- --base http://host:port [--out report.md] [--ticks N]`
 *
...
export async function runConformanceTool(
  ...
  const report = await conformance({ baseUrl: options.base, ticks: options.ticks });
